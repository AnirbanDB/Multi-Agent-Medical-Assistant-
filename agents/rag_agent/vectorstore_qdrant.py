...
class VectorStore:
    ...
    def __init__(self, config):
        ...
        self.url = config.rag.url
        self.api_key = config.rag.api_key

        # Use remote Qdrant cloud server if a URL is provided, else fall back to local on-disk DB
        if self.url:
            self.logger.info(f"Connecting to remote Qdrant server at {self.url}")
            self.client = QdrantClient(url=self.url, api_key=self.api_key)
        else:
            self.logger.info(f"Using local Qdrant DB at {self.vectorstore_local_path}")
